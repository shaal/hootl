...
0. **PREFLIGHT** (once per task) -- Claude validates the task's clarity, scope, and reproducibility. Produces `understanding.md` for context bridging. Based on the verdict:
   ...
   - `too_broad` → subtasks auto-created via `backend.createTask()` in `ready` state; parent moves back to `ready` with subtask IDs as dependencies (so it's picked up again after subtasks complete). `understanding.md` is deleted so preflight runs fresh on re-run. Falls back to `blocked` if no subtasks provided.
